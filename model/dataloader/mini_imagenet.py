...
IMAGE_PATH = './datasets/miniImageNet/mini/images'
SPLIT_PATH = './datasets/miniImageNet/mini/split'
...
class MiniImageNet(Dataset):
    ...
    def __init__(self, setname, args, augment=False):
        ...
        # what is the cache_path for?
        cache_path = osp.join( CACHE_PATH, "{}.{}.{}.pt".format(self.__class__.__name__, setname, im_size) )

        self.use_im_cache = ( im_size != -1 ) # not using cache; -1 for no cache, and -2 for no resize, only for MiniImageNet and CUB
        ...
        self.num_class = len(set(self.label))  # set() can split the labels according to value
        image_size = 84
    ...
    def parse_csv(self, csv_path, setname):
        lines = [x.strip() for x in open(csv_path, 'r').readlines()][1:]  # skip [0] 'column title'; strip() removes space

        data = []
        label = []
        lb = -1
        
        self.wnids = []  # record all the original labels' id, default total num is 64

        # use tqdm to print data processing
